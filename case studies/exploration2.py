...
df_column = pd.read_csv("data/crimenes.csv")
df = pd.read_csv("data/archivo_salida2.csv")
...
df['var_amount_media'] = df['var_amount'].apply(calcular_punto_medio)

amount_mapping = {range_str: i for i, range_str in enumerate(df['var_amount'].unique())}
df['var_amount_numeric'] = df['var_amount'].apply(lambda x: amount_mapping[x])
...
# imprimir df completo (todas las filas)
...
for i in range(2):
    ...
    def update_figure(selected_mes, i=i):
        ...
        fig = px.bar_polar(filtered_df, r="var_frequency", theta="var_crime",
                           color="var_amount", template="plotly_dark",
...
def update_parallel_coordinates(*args):
    ...
    filtered_df = df[df['mes'] == selected_mes]
    directions = filtered_df['var_crime'].unique()
    direction_mapping = {direction: i for i, direction in enumerate(directions)}
    direction_values = filtered_df['var_crime'].map(direction_mapping)

    fig = go.Figure(data=go.Parcoords(
        line=dict(color=filtered_df['var_amount_numeric'], colorscale='Tealrose'),
        dimensions=[
            ...
            dict(range=[0, len(directions)-1],
                 ...
                 label='Crime', values=direction_values,
                 ticktext=list(direction_mapping.keys())),
            dict(range=[filtered_df['var_amount_numeric'].min(), filtered_df['var_amount_numeric'].max()],
                 label='Amount [section]', values=filtered_df['var_amount_numeric']),
            # Agregar var_district 
            dict(range=[filtered_df['var_district'].min(), filtered_df['var_district'].max()],
                    label='district', values=filtered_df['var_district'])
...
def update_radar_chart(mes1, mes2):
    ...
    medias_mes1 = [df_mes1['var_frequency'].mean(), df_mes1['var_amount_numeric'].mean(), df_mes1['var_district'].mean()/5]
    medias_mes2 = [df_mes2['var_frequency'].mean(), df_mes2['var_amount_numeric'].mean(), df_mes2['var_district'].mean()/5]

    # Categorías para el gráfico de radar
    categories = ['Frecuencia', 'Amount', 'District (more)']
...
def update_weekly_bar_chart(mes1, mes2):
    ...
    weekly_means = df_filtered.groupby(['mes', 'semana'])[['var_frequency', 'var_amount_media']].mean().reset_index()
